...
4. **Codice del progetto** — `assets/shared/*.js` con le feature comuni (progresso di studio, preferiti, evidenziatore, ricerca, tema) e i vari `assets/styles.css` / `app.js` per aspetto e configurazione di ciascun vault. L'hub ha in più i propri `assets/hub*.js` (landing e backup/QR) e `assets/version.js` (versione **SemVer** dell'hub, mostrata nel footer: si bumpa a ogni push per confermare, a hard-refresh, che il deploy è salito).
