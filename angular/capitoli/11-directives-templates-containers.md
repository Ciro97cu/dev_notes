...
`ElementRef` è un **wrapper Angular** attorno all'elemento DOM su cui il componente o la direttiva è applicata. Si ottiene tramite `inject`:

```ts
private host = inject(ElementRef<HTMLElement>);
```

La proprietà `nativeElement` espone l'oggetto DOM grezzo — lo stesso che otterresti con `document.querySelector`:

```ts
this.host.nativeElement           // HTMLElement
this.host.nativeElement.style.color = 'red';    // manipolazione diretta
...
Un **template** (`<ng-template>`) definisce markup che Angular **non renderizza subito**: resta "in attesa" e lo si istanzia quando serve. Un **container** è invece il punto dove quel template può essere istanziato — la "presa" del DOM in cui infilare le view generate: Angular mette ogni componente e ogni markup statico in un container invisibile, che si recupera (es. via [[glossario#dependency-injection-di|DI]] come `ViewContainerRef`) per aggiungervi elementi.
...
`inject(ViewContainerRef)` ottiene il container dell'host, e `clear()` va chiamato prima di `createEmbeddedView` perché quest'ultimo **accoda** soltanto; l'`effect` aggiorna poi la view quando `template()` o `context()` cambiano. L'oggetto restituito da `createEmbeddedView` dà accesso ai nodi via `rootNodes`: con la microsyntax (`*appTableField`) c'è **sempre un solo** root element, mentre con un `<ng-template>` esplicito possono essercene più di uno:
