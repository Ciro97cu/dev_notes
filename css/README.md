...
| 03 | [Pseudo-classi & pseudo-elementi](docs/03-pseudo-classi-elementi.md) | `:hover`, `:nth-*`, `:not()`, `:has()`, `::before/after`, `::marker`, `@counter-style` |
...
| 06 | [Unità, valori & funzioni](docs/06-unita-valori-funzioni.md) | Unità (incl. `lh`/`rlh`), `calc()`/`clamp()`, custom properties, DPR |
| 07 | [Colori](docs/07-colori.md) | hex/rgb/hsl, `oklch()`, `color-mix()`, relative colors, `contrast-color()`, `color-scheme` |
| 08 | [Testo & font](docs/08-testo-font.md) | `font` shorthand, `@font-face`, generic family (`math`), text-decoration, `text-wrap` |
| 09 | [Display & posizionamento](docs/09-display-posizionamento.md) | display, position, z-index, overflow, `content-visibility`, float |
...
| 15 | [Transizioni & animazioni](docs/15-transizioni-animazioni.md) | `transition`, `@keyframes`, timing, reduced-motion, view transitions |
