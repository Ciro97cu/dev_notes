...
Attenzione a cosa fa cosa: il file `.nvmrc` **resta nel progetto** (di norma versionato in git) e fissa *quale* versione Node vuole, una volta per tutte, ma **non attiva nulla da solo**. A ogni nuova shell si esegue `nvm use` senza argomenti nella cartella del progetto: legge `.nvmrc` e attiva quella versione (un'attivazione che, come ogni `nvm use`, vale **solo per quella shell**). Per non digitarlo ogni volta si aggiunge al profilo di shell un **hook** che lo fa da sé appena si entra nella cartella; cos'è un *hook* e come si scrive è spiegato nel vault Terminale, <a href="../terminale/#/docs/06-file-configurazione-shell?id=hook-far-girare-codice-in-automatico" target="_blank" rel="noopener">Hook</a>.
