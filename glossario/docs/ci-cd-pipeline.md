...
L'**integrazione continua** (*Continuous Integration*) è la pratica di **far confluire spesso** le modifiche dei vari sviluppatori nel **ramo comune** del repository (il `main`), verificando **in automatico** a ogni confluenza che il codice unito regga. Il termine nasce da un problema storico: quando ognuno lavorava per settimane in isolamento e si «integrava» tutto solo alla fine, riaffiorava una massa di conflitti (l'*integration hell*); farlo di continuo, un pezzetto per volta, li fa emergere subito, quando sono ancora piccoli. In questo contesto «integrare» ha un senso ristretto, cioè **unire il codice di più persone nello stesso progetto**, da tenere distinto dall'integrazione fra sistemi diversi come un frontend e un backend, che è un fatto di *runtime* (applicazioni già in esecuzione che dialogano, di solito via API). Il «team» è chi condivide il repository, spesso un solo gruppo di lavoro; la pratica vale anche con un unico sviluppatore, perché il lavoro con cui ci si integra è quello già presente sul `main`.

A ogni `git push` (o apertura di una pull request) un server prende il codice aggiornato e ci fa girare sopra una batteria di controlli: installa le dipendenze, passa il *linter* (il controllo di stile), esegue i test, costruisce l'applicazione. Se uno di questi passi fallisce, chi ha spinto la modifica lo sa nel giro di minuti, quando l'errore è ancora piccolo e fresco, invece di scoprirlo settimane dopo mescolato al lavoro di altri. Il valore della CI è tutto qui: **accorciare la distanza tra l'errore e la sua scoperta**.

## CD: consegna continua

La **consegna continua** (*Continuous Delivery*) è la metà che viene dopo. Ciò che la CI ha costruito nell'ultimo passo è l'**artefatto**: il pacchetto pronto al rilascio, cioè l'output della *build* impacchettato in una forma installabile (un bundle di file statici, un'immagine container, un `.jar`, uno zip) e non il codice sorgente. Quando i controlli della CI sono verdi, quello **stesso** artefatto viene **rilasciato in automatico** negli ambienti, di solito prima uno di prova (*collaudo* o *staging*) e poi la produzione, cioè il sito che vedono gli utenti.

A separare i due gradi è **solo il gradino della produzione**. Nella *continuous delivery* la messa in produzione resta dietro un'**approvazione manuale**: l'automazione prepara e distribuisce tutto fino allo staging, poi una persona dà l'ultimo via. Nella *continuous deployment* anche quel passo parte da solo, senza intervento umano. In entrambe l'evento che avvia il tutto è lo stesso (un push o un merge sul ramo), e distribuire in automatico su sviluppo e collaudo è normale nei due casi: la differenza sta unicamente in chi decide quando si va in produzione.
