...
L'**integrazione continua** (*Continuous Integration*) è la pratica di **far confluire spesso** le modifiche dei vari sviluppatori nel **ramo comune** del repository (il `main`), invece di lasciare che il ramo di ciascuno diverga per settimane, verificando **in automatico** a ogni confluenza che il codice unito regga. Il nome viene da un dolore storico: quando ognuno lavorava per mesi in isolamento e solo alla fine si tentava di «integrare» tutto insieme, riaffiorava una montagna di conflitti (l'*integration hell*). Integrare **di continuo**, un pezzetto per volta, fa emergere subito conflitti e rotture, quando sono ancora piccoli.

«Integrare», qui, vuol dire **unire il codice di più persone nello stesso progetto**, non collegare sistemi diversi come frontend e backend (quello è un'altra cosa, che avviene fra applicazioni già in esecuzione, di solito via API). Il «team» è semplicemente **chi condivide quel repository**, che può benissimo essere un solo gruppo, per esempio quello di frontend con un repo di solo codice front-end; la CI ne integra i commit dei vari membri. Ha senso anche da soli: «l'altro lavoro» con cui integrarsi è semplicemente quanto già sul `main`.
